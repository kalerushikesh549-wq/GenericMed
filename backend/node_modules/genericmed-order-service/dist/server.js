...
async function connectMongo() {
    ...
    try {
        mongoClient = new mongodb_1.MongoClient(connectionString, { serverSelectionTimeoutMS: 8000 });
...
async function persistOperationalRecord(collection, record) {
    if (!mongoDatabase)
        return;
    try {
...
const PAYMENT_METHODS = new Set(['card', 'apple_pay', 'google_pay', 'hsa_fsa']);
const paymentTransactions = new Map();
const wholesaleInventory = new Map([['#CP-9021', 45000]]);
const coldChainShipments = new Map();
function calculateSplit(amountCents, courierPayoutCents = 0) {
...
function priceTierFor(quantityUnits) {
    if (quantityUnits >= 20000)
        return 'tier_3';
    if (quantityUnits >= 5000)
        return 'tier_2';
    return 'tier_1';
}
...
app.post('/api/v1/payments/intents', (req, res) => {
    const { amountCents, paymentMethodType, paymentMethodId, courierPayoutCents = 0 } = req.body;
    ...
    try {
        ...
        const id = `pi_demo_${crypto_1.default.randomUUID().replace(/-/g, '')}`;
...
app.get('/api/v1/payments/:paymentIntentId', (req, res) => {
...
app.post('/api/v1/insurance/adjudications/quote', (req, res) => {
    const { cashPriceCents, payerName = 'Coverage estimate', brandCopayCents } = req.body;
    ...
    const quote = { status: process.env.CLEARINGHOUSE_MODE === 'live' ? 'adjudicated' : 'estimated', payerName: String(payerName), brandCopayCents: brandCopay, genericCopayCents: genericCopay, cashPriceCents, patientNetSavingsCents: Math.max(0, brandCopay - cashPriceCents), expiresAt: new Date(Date.now() + 15 * 60 * 1000).toISOString() };
    return res.json(quote);
});
app.post('/api/v1/refill-subscriptions', (req, res) => {
    const { prescriptionId, cadenceDays, paymentIntentId } = req.body;
    ...
    return res.status(201).json({ id: `sub_demo_${crypto_1.default.randomUUID().replace(/-/g, '')}`, status: 'active', cadenceDays, nextChargeAt });
});
// Phase 6: PO allocation is guarded by cGMP token presence and available batch inventory.
app.post('/api/v1/wholesale/purchase-orders', (req, res) => {
    const { batchNumber, quantityUnits, hubName, signerLicenseNumber, cgmpToken } = req.body;
    ...
    if (quantityUnits > availableUnits)
        return res.status(409).json({ error: 'Requested allocation exceeds available released batch inventory.', availableUnits });
    ...
    const signatureHash = crypto_1.default.createHash('sha256').update(`${poNumber}|${batchNumber}|${quantityUnits}|${signerLicenseNumber}|${cgmpToken}`).digest('hex');
...
app.post('/api/v1/cold-chain/telemetry', (req, res) => {
    const { shipmentReference, batchNumber, deviceId, temperatureC, humidityPercent, recordedAt } = req.body;
    ...
    const shipment = coldChainShipments.get(shipmentReference) ?? { shipmentReference, batchNumber, deviceId, status: 'in_transit', readings: [] };
    if (shipment.batchNumber !== batchNumber || shipment.deviceId !== deviceId)
        return res.status(409).json({ error: 'Shipment identity does not match its registered batch or sensor.' });
...
app.get('/api/v1/cold-chain/shipments/:shipmentReference', (req, res) => {
    const shipment = coldChainShipments.get(req.params.shipmentReference);
    if (!shipment)
        return res.status(404).json({ error: 'Cold-chain shipment not found' });
